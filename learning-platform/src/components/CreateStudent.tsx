...
export const CreateStudent = () => {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:3000/create-student", {name, email, password});
  ...
  return (
    <div className="bg-[#f4f4f5] flex items-center justify-center h-screen">
      <div className="w-[420px] flex flex-col p-6 bg-white outline-1 outline outline-neutral-200 shadow-md rounded-xl">
        <h1 className="text-2xl font-medium mx-auto">Register as Student</h1>
        ...
        <form className="flex flex-col w-[360px] mt-8" onSubmit={handleSubmit}>
          ...
          <p className="text-sm text-center">Already have account?{" "}<a href="/student-login" className="underline text-blue-600">Login</a></p>
        </form>
      </div>
      <Toaster />
    </div>
